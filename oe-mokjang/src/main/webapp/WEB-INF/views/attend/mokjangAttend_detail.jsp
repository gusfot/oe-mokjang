		...
		<div class="container">
			...
			<div>
				<form id="mokjangAttendForm">
					<input type="hidden" name="mokjangAttendSeq" value="${mokjangAttend.mokjangAttendSeq }" />
				<div class="content_wrap">
					<div class="content">
						<a href="#" class="ui-btn ui-btn-b ui-corner-all ui-icon-check ui-btn-icon-left ui-mini" onclick="javascript:attend.mokjang.modify();">목장보고서 수정</a>
									...
									<c:forEach var="user" items="${mokjangUsers}" varStatus="i" >
										<tr data-userseq="${i.index}" >
											<td>
												...
												<input type="hidden" class="form-control" name="attends[${i.index}].userSeq" value="${user.userSeq}" >
											</td>
											<c:forEach var="item" items="${attendItems}" varStatus="j">
												<td>
													<label class="attend_chk">
														<input type="checkbox" data-user="user${i.index}" data-weight="${item.weight}" data-mini="true" name="attends[${i.index}].attendItemHistList[${j.index}].itemValue" value="1"  <c:if test="${mokjangAttend.attends[i.index].attendItemHistList[j.index].itemValue eq 1}">checked="checked"</c:if> ><c:if test="${item.weight > 0}">+${item.weight}</c:if><c:if test="${item.weight < 0}">${item.weight}</c:if>
														<input type ="hidden" name="attends[${i.index}].attendItemHistList[${j.index}].itemCode" value="${mokjangAttend.attends[i.index].attendItemHistList[j.index].itemCode}" />
														<input type ="hidden" name="attends[${i.index}].attendItemHistList[${j.index}].attendItemHistSeq" value="${mokjangAttend.attends[i.index].attendItemHistList[j.index].attendItemHistSeq}" />
														</label>
													</td>
											</c:forEach>
											<td>
												<a href="#" class="ui-btn ui-corner-all ui-mini"><span id="attends[${i.index}].point">${mokjangAttend.attends[i.index].point}</span> P</a>
							...
							<table class="content_table">
								<tr>						
									<th>금일합계점수</th>
									<td>
										<p class="today_point"><a href="#" class="ui-btn ui-corner-all ui-mini"><span id="point">${mokjangAttend.point }</span>P</a></p>
										<input type="hidden" name="point" value="${mokjangAttend.point }" />
									</td>							
								</tr>
								<tr>
									<th>누적점수</th>
